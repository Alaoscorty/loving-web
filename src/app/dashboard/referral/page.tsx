...
export default function ReferralPage() {
  ...
  const referralCode = userProfile?.uid?.slice(0, 8).toUpperCase() || 'LOVING-2026';
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      if (userProfile && !userProfile.completedQuests?.includes('invite_friend')) {
        await completeQuest({ firestore, userId: userProfile.uid, questId: 'invite_friend', points: 20 });
      }
    } catch (error: any) {
      toast({
        variant: 'destructive',
        title: 'Erreur',
        description: error.message || 'Impossible de copier le lien.',
      });
    } finally {
      ...
      <Card className="rounded-[2.5rem] border-none bg-gradient-to-br from-primary/10 via-card to-accent/10 shadow-2xl overflow-hidden relative border border-white/5">
        ...
        <CardContent className="p-8 md:p-12 pt-0 space-y-6">
          ...
          <Alert className="bg-primary/5 border-primary/20 rounded-2xl">
            ...
            {isAmbassador ? (
              <p className="text-sm mt-2">Félicitations ! Vous avez débloqué le badge exclusif <span className="font-bold">"Ambassadeur Loving"</span> et gagné 100 XP bonus. 🎁</p>
            ) : (
              <p className="text-sm mt-2">Parrainez 5 amis avec RDV validé pour débloquer le badge exclusif <span className="font-bold">"Ambassadeur Loving"</span> et gagner 100 XP bonus.</p>
            )}

            <div className="space-y-3 mt-4">
              <div className="flex flex-wrap justify-between gap-2 text-xs font-bold uppercase tracking-widest text-muted-foreground">
